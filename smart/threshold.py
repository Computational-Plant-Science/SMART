...
def simple_threshold(image: np.ndarray, threshold: int = 90, invert: bool = False) -> np.ndarray:
    ...
    _, image = cv2.threshold(image.copy(), threshold, 255, cv2.THRESH_BINARY_INV if invert else cv2.THRESH_BINARY)
    return image
...
def otsu_threshold(image: np.ndarray, invert: bool = False) -> np.ndarray:
    image = cv2.createCLAHE(clipLimit=3).apply(image.copy())
